...
# Train model
@st.cache_resource
def train_model(_processor):
    """Train ML model - simplified stratified split for better accuracy"""
    spark = init_spark()
    
    # Split data: 90% train, 10% test (maximum training data for high accuracy)
    # Split each class separately to keep balanced distribution
    positive = _processor.processed_df.filter(col('two_year_recid') == 1)
    negative = _processor.processed_df.filter(col('two_year_recid') == 0)
    
    pos_train, pos_test = positive.randomSplit([0.90, 0.10], seed=42)
    neg_train, neg_test = negative.randomSplit([0.90, 0.10], seed=42)
...
def show_model_performance(processor):
    ...
    st.info("Using Random Forest Classifier for recidivism prediction")
    
    if st.button("Train Model"):
        with st.spinner("Training Random Forest model..."):
            try:
                predictor, predictions, metrics, fairness_metrics, test_df = train_model(processor)
